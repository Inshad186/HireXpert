  ...
  const [dashboardStats, setDashboardStats] = useState({
    ...
    const getStatusColor = (status: string) => {
      switch(status){
        ...
        default : return "bg-blace text-red-800"
      }
    }
  ...
  return (
    <div className="p-6 text-white">
      ...
      <div className='flex items-center mb-6'>
        ...
        <button 
        className='px-4 py-2 bg-gray-900 rounded-r-md'>Search
        </button>
          ...
          <table className='w-full'>
            ...
            <tbody>
              {order?.length > 0 ? (
                ...
                <tr key={item._id}>
                  <td className='px-6 py-4 font-medium text-sm text-white'>{index+ "0"+ 1}</td>
                  ...
                  <td className='px-6 py-4 text-sm '>
                    ...
                    <td className='px-6 py-4 text-base font-medium'>
                      ...
                      onClick={() => setViewModal(true)}>view
                      </button>
                    </td>
                </tr>
              ))
      ...
      {viewModal && (
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center p-4 z-50">
          ...
          {/* view Modal */}
            ...
            {order.map((item, index) => (
              <div key={item._id || index} className="grid grid-cols-2 gap-6 bg-white p-4 rounded-lg border">
                <div>
                ...
                <div>
                  ...
                  <p className="text-lg font-semibold text-gray-800">{item.client.name}</p>
                </div>
                <div>
                  <p className="text-gray-600 text-sm">Freelancer</p>
                  <p className="text-lg font-semibold text-gray-800">{item.freelancer.name}</p>
                </div>
                <div>
                  <p className="text-gray-600 text-sm">Project</p>
                  <p className="text-lg font-semibold text-gray-800">{item.gig.title}</p>
                </div>
                <div>
                  <p className="text-gray-600 text-sm">Amount</p>
                  <p className="text-lg font-semibold text-green-600">{item.gig.pricing.basic.price}</p>
                </div>
                <div className="col-span-2">
                  <p className="text-gray-600 text-sm">Requirements</p>
                  <p className="text-gray-800 mt-2 bg-gray-100 p-3 rounded capitalize">{item.requirements}</p>
                </div>
              </div>
            ))}
          </div>
